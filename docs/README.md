...
| **M6** ✅ | [Money, and the five things that were wrong](21-m6-notes.md) | **touching billing, promo codes or templates.** What M6 shipped on 31 August, and the five defects only a running system produced: a downgrade the API announced and rolled back, a rejected row that poisoned the *next* transaction (a redelivered webhook would have 500ed forever), and a lazy load after a flush. 🔴 Read §2.2 before writing any `except IntegrityError` around a savepoint — and before trusting this suite about request transactions, which it structurally cannot see |
