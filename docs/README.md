...
| **Uploads & CI** 🟢 | [Multipart uploads, and the MinIO the CI never started](19-multipart-and-ci.md) | **touching the upload flow, or the CI's services.** A second audit: the multipart completion passed an ETag where S3 wanted the upload id — real, and larger than reported, because the browser refused files over 100 MB so nothing could reach it. Both halves ship. The MinIO health check it flagged was fine; the CI's MinIO was never running, for an unrelated reason a health check cannot fix |
| **M6** ✅ | [Money, and the six things that were wrong](21-m6-notes.md) | **touching billing, promo codes or templates.** What M6 shipped on 31 August, and the six defects only a running system produced: a downgrade the API announced and rolled back, a rejected row that poisoned the *next* transaction (a redelivered webhook would have 500ed forever), and a lazy load after a flush — and, once the real webhook key arrived, a brand-new account that paid and stayed on free because idempotency compared the period without the plan (§2.5). 🔴 Read §2.2 before writing any `except IntegrityError` around a savepoint — and before trusting this suite about request transactions, which it structurally cannot see |
| **M6 prep** 🟢 | [M6 readiness](20-m6-readiness.md) | **starting M6, and written for a developer picking it up cold.** More is built than the checklist suggests — the tables, the ledger and the plan gating all ship — so the missing part is the outside world. It carries 🔴 **four traps** worth reading before writing a line (the `is_public` column nothing reads, a queue priority between Celery's bands, the two `plans.py` dictionaries that raise `KeyError`, and the renewal that must never sweep `topup`), the two blockers that are not code, and 🟠 the cost-per-job number that decides whether a $3.99 plan makes money |
| **The claim** 🟢 | [The media_assets claim](18-media-asset-claim.md) | **touching ingest, or anything that starts a worker on an asset.** The gap M4 left open, closed before M5: `media_assets` now has the atomic claim `jobs` has had since M4, so a stuck `probing` asset is recovered rather than merely logged. Read §2 before assuming the mirror is exact — the guard could not be `WHERE status='probing'`, because an asset is already `probing` when its message is sent |
