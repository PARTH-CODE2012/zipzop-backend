...
### The competitor-research notes, 30 August

The project lead sent three points from CapCut/InShot/VN reviews. Where they land:

- 🔴 **Hindi/Devanagari captions — folded into M5, and it was not a small check.** It needed two independent things, and a Latin test cannot see either. **Shaping**: `ि` is written *before* the consonant it follows and `न` + `्` + `द` is one conjunct, and `drawtext` — the obvious filter, and what a caption renderer reaches for first — goes through libfreetype with no shaping at all. Captions render through **libass**, which shapes via HarfBuzz. **Coverage**: neither Arial nor DejaVu has a Devanagari block, so perfect shaping still gives empty boxes; `fonts-noto-core` is now installed in the backend image. `tests/test_render_text.py` proves the difference by rendering the same Hindi string both ways and measuring that they differ, rather than asserting a configure flag
- **Billing — M6, unchanged.** Clear pricing and one-click cancel are interface and policy over a schema that already exists (`plans`, `subscriptions`, `payments` are seeded and migrated since M2). `POST /billing/cancel` is already in the contract §10 as *"cancel at period end"*, which is the no-hidden-charges behaviour asked for
- **Privacy — cheap because it is what was already built, not a new feature.** No social feed exists or is planned; the scope doc has no sharing surface. Worth *stating* rather than building: a privacy page saying what is collected. One thing to check before claiming "minimal data collection" — the transcription model is self-hosted (decided 20 August, `docs/11-m4-notes.md` §1), so speech never leaves our infrastructure, which is a stronger claim than any competitor can make and is currently written down nowhere a customer sees
...
- [x] 🔴 **The renderer can read the LUTs** — done 28 August, M5's first task. The five `.cube` files moved to `backend/app/assets/luts/`, which is inside the build context; `app/services/luts.py` resolves a look to a path and refuses anything not on disk (the name arrives from a timeline document, so a traversal is client input). The browser's copy under `frontend/public/luts/` is generated by `frontend/scripts/sync-luts.mjs`, run by `pnpm dev|build|test` and by `make luts`, and gitignored — two committed copies of the same bytes is a day when they differ. `tests/test_luts.py` asserts every name in `LOOKS` has a grade **and hands each one to FFmpeg's `lut3d`**, because a `.cube` our own parser likes is not evidence FFmpeg does
- [x] **`ExportInput`, and `export` opened in `PHASE_1_TOOLS`** — `timelineVersion` plus a preset of named values (`720p`/`1080p`/`2160p` — exactly the ladder the seeded plan ceilings name, so no tier is invented in a schema; `9:16`/`1:1`/`16:9`; `draft`/`standard`/`high` mapping to CRF here rather than travelling in the payload, where an old queued job would render at the old meaning). The only input with no `assetId`: an export is of a *project*, and a request without `projectId` is refused by the request model so the client gets a 422 naming the field rather than a 404 for a project it never mentioned
- [x] **`POST /jobs {tool: export}` — `409` on a stale `timelineVersion`, `403` on a resolution above the plan.** `quote()` gained a second branch: an export is priced from the project's duration, not an asset's. `!=` and not `<` — a client ahead of the server is at least as wrong as one behind. The ceiling is *carried* rather than raised so `POST /jobs/estimate` reports it in `blockedBy` and the button can be greyed out with a reason; and it is checked before affordability, because being told to buy credits for a render the plan forbids at any price is the wrong sentence. `requiredPlan` names the upgrade. 12 tests in `tests/test_export_jobs.py`
- [x] **The filter graph, as a pure function** — `app/services/render_graph.py`. Trims, speed (`atempo` *chained* past 2x, because the filter caps at 2.0 and the document allows 4.0 — clamping would run a clip's audio at half the speed of its picture), crop, flip, quarter-turn rotation, the grade, scale-and-pad, concat, watermark, and music delayed to where the user put it on the timeline. 16 tests, two of which render real media and probe the result — an exit code of 0 only says FFmpeg did not crash
- [x] **The grade is `split` + `lut3d` + `blend`, not `lut3d` alone** — `strength` is a fraction and `lut3d` has no opacity. Applying it flat and calling 0.75 close enough would make *every* graded export differ from the preview, which composites the same LUT at the same fraction in a shader
- [x] 🔴 **`drawtext` needs a font *file*, not a family** — found by the watermark test failing with `Fontconfig error: Cannot load default config file` and an exit code that is not an error message. **The same class of problem as the LUTs**: it fails at render time, on a job the user has already paid for. `app/services/fonts.py` resolves a file. ⚠️ **Bundling one in `app/assets/fonts/` is the durable fix** and is a licensing choice, so it is named here rather than made silently; until then it falls back to the platform's own fonts
- [x] `storage.export_key()` — `exports/{user_id}/{job_id}/final.mp4`, keyed by **job** and not by asset: exporting the same project twice gives two files that are both correct, and an asset-keyed path would have the second overwrite the first. `tests/test_storage_keys.py` pins the whole §6.3 layout, which nothing tested before — each prefix carries a retention promise, and a key that drifts does not fail, it quietly gets the wrong lifecycle rule
- [x] **Frame-comparison tolerance decided**: mean absolute error per channel over a downscaled frame, sampled at a handful of playhead positions, with a stated ceiling — the readiness note's own recommendation, taken. The preview composites a 480p proxy and the export uses the original, so an exact match is the wrong target; what the test must catch is *systematic* drift — a LUT at the wrong strength, in the wrong colour space, or not applied
- [x] 🔴 **The comparison is written, and it found a real bug on its first run** — `make parity`, driving a real headless Chrome. Known colours go through **both real implementations**: the exported `FRAGMENT_SRC` and `parseCubeLut` on a GPU context (`/spike/lut-parity`), and the export renderer's own grade filter, read from `app.scripts.grade_filter` so there is one implementation and not a copy. Five grades × three strengths, MAE **0.79-1.33 levels** against a ceiling of 3, worst single channel 3 against 8
- [x] 🔴 **The bug it found: every partial-strength export applied the grade at `1 - strength`.** In FFmpeg's `blend` the *first* input is the top layer and is the one `all_opacity` applies to; the graph passed the ungraded copy first, so the opacity landed on the original. On a desaturating look at 0.6 the blue channel was **41 levels out**. Nothing else could have caught it: full strength skips the blend, so every graph test and every render test passed. Fixed and pinned by a test on the filter string
- [x] **Swatches rather than whole frames, deliberately.** The preview composites a 480p proxy and the export uses the original, and H.264 is lossy on both sides, so a photograph comparison measures mostly compression — noise that either swamps a real drift or forces a tolerance that catches nothing. The one thing the two implement *separately* is the LUT, its interpolation and the mix, so that is what is measured, on flat colours where one level is visible
- [x] **Transitions decided: neither side draws them in M5.** The readiness note framed this as a choice between teaching the preview transitions and letting the export render them with the editor saying so. There is a third, and it is the one that makes the milestone's promise *true* rather than qualified: **the renderer does not draw them either**, so preview and export agree exactly. The document keeps carrying `transitionIn`/`transitionOut` — it is valid, validated and round-trips — and every join is a cut on both sides. Teaching the WebGL engine crossfades means overlapping clips in time, which invariant 1 forbids, so it is a document change and not an engine change
- [ ] 🟠 **The editor must say so.** A clip carrying a transition currently looks like it will get one. Small, and it is the half of the decision that is not done — a label on the clip, or the inspector saying "not shown in this version"
...
- [x] **Render worker, and `_enqueue` routed by family** — `app/workers/tasks/render.py` on the `render` queue, which M4 already separated from `analysis` precisely so a render holding a worker for minutes could not block a caption that takes seconds. `_enqueue` sent *everything* to `run_analysis` until this landed, so an export was charged for and then failed
- [x] **`render_pipeline.py`** — claim, fetch the **originals** (never the proxies: the proxy is 480p so the browser can scrub, and exporting from it would cap every render at the preview's resolution), render, upload to `storage.export_key`, create the output `media_assets` row with `derived_by_job_id`, settle. Refunds on failure, §5.4. Fewer retries than analysis on purpose: a retry costs a full transcode, so anything that is not a storage blip fails permanently on the first attempt
- [x] **Progress from FFmpeg's `-progress`**, not a clock — `out_time_ms` against the timeline's own duration. ⚠️ It is **microseconds despite the name**, a long-standing quirk; reading it as milliseconds makes a 60-second render report 0.1%. Capped at 90 until the upload finishes, because a bar sitting at 100 while something is still happening is the same lie as one driven by a timer
- [x] **Watermark from the plan, never from the input** — contract §6.2 is explicit the client cannot ask for it to be left off, and an input field it could set would be exactly that
- [x] **`GET /catalog/luts`** — promised by §4.4 since M2 and never implemented. The client hardcoded the five names and a test compared two copies of the same list by hand; the server is the single source now. Public, because the pricing page has to show what the product does before anyone signs up
- [x] **The export dialog** — a header action rather than a rail mode (the rail is one mode at a time and each acts on a selection; an export acts on the project). Price on the button from `/jobs/estimate`, which is the same function that charges; `blockedBy` shown as a sentence naming the plan rather than a greyed-out button with no explanation; the timeline version read at submit rather than at open, so an autosave mid-thought does not become a 409
