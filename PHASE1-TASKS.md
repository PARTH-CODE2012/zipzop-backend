...
- [ ] 🔴 **The comparison itself is NOT written.** It is the one thing that turns *"it looks exactly like the preview"* from a claim into a checked fact, and it needs the browser's WebGL output next to FFmpeg's, which means driving a real browser — the same harness `make e2e` needs and which has still never run. **M5 is not closed until this exists**
