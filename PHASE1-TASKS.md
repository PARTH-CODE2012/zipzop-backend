...
- [ ] 🔴 **Render worker — NOT BUILT, and `export` is open without it.** `POST /jobs` accepts an export, prices it and reserves the credits, but `_enqueue` still sends every job to `run_analysis`, which has no `export` branch: the job fails with `UNSUPPORTED_MEDIA` and refunds. Nothing in the frontend calls export yet, so no user can reach it — but **do not treat export as usable until this lands**. Needs `app/workers/tasks/render.py`, routing in `_enqueue` by family, downloading the **originals** (not the proxies — the proxy is 480p and exists so the browser can scrub; exporting from it would cap every render at the preview's resolution), uploading to `storage.export_key`, creating the output `media_assets` row and setting `output_asset_id`
